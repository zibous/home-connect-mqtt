...
class HCSocket:
	...
	def decrypt(self,buf):
		...
		if len(buf) < 32:
			print(__name__, "Short message?", buf.hex(), file=sys.stderr)
			return None
		if len(buf) % 16 != 0:
			print(__name__, "Unaligned message? probably bad", buf.hex(), file=sys.stderr)
		...
		if their_hmac != our_hmac:
			print(__name__, "HMAC failure", their_hmac.hex(), our_hmac.hex(), file=sys.stderr)
			return None
		...
		pad_len = msg[-1]
		if len(msg) < pad_len:
			print(__name__, "padding error?", msg.hex())
			return None
