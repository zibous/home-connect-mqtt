...
class HCDevice:
	...
	def recv(self):
		try:
			...
			print(__name__, "receive error", e, traceback.format_exc())
			return None

		try:
			return self.handle_message(buf)
		except Exception as e:
			print(__name__, "error handling msg", e, buf, traceback.format_exc())
			return None
	...
	def handle_message(self, buf):
		...
		if "code" in msg:
			...
			or resource == "/ro/values":
				...
				# print(now(), __name__, "services", self.services)
...
#					self.get("/ei/deviceReady", version=self.services["ei"]["version"], action="NOTIFY")
				...
				print(now(), __name__, "Unknown message", msg)
